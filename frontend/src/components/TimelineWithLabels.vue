  ...
  d3.timeline = function() {
    ...
    var hover = function() {},
      mouseover = function(d) {
        ...
        let labelText = document.createTextNode(d.label_label)
        labelP.appendChild(labelText)
        card.appendChild(labelP)
        let dataP = document.createElement('P')
        let dataText = document.createTextNode(d.label)
        dataP.appendChild(dataText)
        card.appendChild(dataP)
        document.getElementById('chart-hover').appendChild(card)
      },
      mouseout = function() {
        var myNode = document.getElementById('chart-hover')
    ...
    var appendLabel = function(gParent, yAxisMapping, index, hasLabel, datum) {
      // console.log('append label') --> left label
      ...
      g.each(function(d) {
        ...
        d.forEach(function(datum, index) {
          ...
          g.selectAll("svg").data(data).enter()
            ...
            .attr("width", function() {
              // console.log('width:', (d.ending_time - d.starting_time) * scaleFactor)
            ...
            .on("mouseout", function(d, i, datum) {
              mouseout(d, i, datum);
            })
            // .on("click", function(d) {
            //   // click(d, index, datum);
  ...
  methods: {
    timelineLabelColor () {
      ...
      var chart = d3.timeline()
        .beginning(this.beggining.valueOf()) // we can optionally add beginning and ending times to speed up rendering a little
        .ending(this.ending.valueOf())
        ...
        .rotateTicks(45);
      d3.select(this.selector).append("svg").attr("width", width)
        .datum(this.data).call(chart);
    }
  }
}
</script>
