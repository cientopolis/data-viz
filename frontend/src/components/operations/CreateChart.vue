    ...
    >
      <div slot="selectChartType" style="margin-top: 20px;">
        ...
        >
          ...
          >{{chartType.title}}</a-select-option>
        </a-select>
        <a-select v-else style="width: 200px" @change="handleChartChange">
          <a-select-option
            v-for="(chartType, index) in chartTypes"
            :key="index"
            :value="chartType.value"
        ...
        <a-alert v-if="columnsError" :message="columnsError" type="error" banner />
      </div>
      <div slot="selectColumns">
        <div v-if="selectedChartType">
          <span style="margin: 10px;">Selecciona la/s columna/s para tu gráfico.</span>
          <a-alert style="margin-bottom: 10px;" :message="selectedChartType.instruction" banner />
          <a-row>
            <div v-for="(field, index) in selectedChartType.fields" :key="index">
              <span>{{field.name}}</span>
              <a-select v-if="field.max == 1" v-model="form[field.model]" style="width: 100%;">
                <a-select-option
          ...
          for (let i = 0; i < this.selectedChartType.fields.length; i++) {
            ...
            if (field.max == 1) {
              ...
              if (this.form[field.model]) {
                let selectedColumns = this.columns.filter(
                  column =>
                    this.form[field.model].indexOf(
      ...
      for (let i = 0; i < this.selectedChartType.fields.length; i++) {
        ...
        if (field.max == 1) {
          ...
          if (
            selectedColumn &&
            field.type.length > 0 &&
            field.type.indexOf(selectedColumn.type) < 0
          ) {
            isValid = false;
          ...
          if (
            field.required == true &&
            (this.form[field.model] == undefined ||
              this.form[field.model].length == 0)
          ) {
            isValid = false;
            message = `El campo ${field.name} es requerido`;
            break;
          ...
          if (this.form[field.model] !== undefined) {
            let selectedColumns = this.columns.filter(
              column =>
                this.form[field.model].indexOf(this.columns.indexOf(column)) >=
                0
            );
